...
prost-types = ""
prost-reflect = ""
twurst-client = { version = "", features=["reqwest-013"] }
...
async fn main() {
    let twirp_client = TwirpHttpClient::new_using_reqwest_013("http://example.com/twirp");
...
```rust,ignore
use twurst_client::{TwirpHttpClient, Reqwest013Service};
...
fn main() {
    let twirp_client = TwirpHttpClient::new_with_base(
        ServiceBuilder::new()
            ...
            .service(Reqwest013Service::from(reqwest::Client::new()))
    );
}
```
...
- `reqwest-013` allows to use [`reqwest` 0.13](https://docs.rs/reqwest/0.13/) HTTP implementation.
