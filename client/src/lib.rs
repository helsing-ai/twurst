...
impl<S: TwirpHttpService> TwirpHttpClient<S> {
    ...
    /// Used internally by the generated code.
    #[cfg(feature = "twurst-stream")]
    pub async fn call_server_streaming<I: ReflectMessage, O: ReflectMessage + Default>(
        &self,
        path: &str,
        request: &I,
    ) -> Result<impl Stream<Item = Result<O, TwirpError>>, TwirpError> {
        ...
        self.extract_streaming_response(response).await
    }
    ...
    async fn extract_response<T: ReflectMessage + Default>(
        ...
        // Success
        let content_type = ContentType::from_headers(response.headers())?;
        let body = collect_body(response.into_body()).await?;
        match content_type {
            ContentType::Protobuf => T::decode(body.aggregate()).map_err(|e| {
                TwirpError::wrap(
                    TwirpErrorCode::Malformed,
            ...
            ContentType::Json => json_decode(&body.to_bytes()),
        }
    }

    #[cfg(feature = "twurst-stream")]
    async fn extract_streaming_response<T: ReflectMessage + Default>(
        &self,
        response: Response<S::ResponseBody>,
    ) -> Result<TwirpResponseStream<S::ResponseBody, T>, TwirpError> {
        // Error
        if response.status() != StatusCode::OK {
            return Err(extract_response_error(response).await);
        }
        ...
        // Success
        let content_type = StreamContentType::from_headers(response.headers())?;
        let body = response.into_body();
        Ok(match content_type {
            StreamContentType::Protobuf => TwirpResponseStream {
                body,
                buffer: BytesMut::new(),
                decoder: StreamResponseDecoder::default(),
            },
            StreamContentType::Json => unimplemented!(),
        })
    }
}
...
pub trait TwirpHttpService: 'static {
    type ResponseBody: Body<Error: Error + Send + Sync + 'static>;
...
impl StreamContentType {
    fn from_headers(headers: &HeaderMap) -> Result<Self, TwirpError> {
        let content_type = headers
            .get(CONTENT_TYPE)
            .ok_or_else(|| TwirpError::malformed("No content-type header"))?;
        if content_type == APPLICATION_PROTOBUF_STREAM {
            Ok(Self::Protobuf)
        } else if content_type == APPLICATION_JSONL {
            Ok(Self::Json)
        } else {
            Err(TwirpError::malformed(format!(
                "Unsupported content type: {}",
...
fn json_decode<T: ReflectMessage + Default>(message: &[u8]) -> Result<T, TwirpError> {
    ...
    dynamic_message.transcode_to().map_err(|e| {
        TwirpError::wrap(
            TwirpErrorCode::Internal,
            format!("Internal error while parsing the JSON response: {e}"),
            e,
        )
    })
}
...
async fn extract_response_error<B: Body<Error: Error + Send + Sync + 'static>>(
    response: Response<B>,
) -> TwirpError {
...
async fn collect_body<B: Body<Error: Error + Send + Sync + 'static>>(
    body: B,
) -> Result<Collected<B::Data>, TwirpError> {
...
#[cfg(feature = "twurst-stream")]
pin_project! {
    pub struct TwirpResponseStream<B: Body, T: ReflectMessage> {
        #[pin]
        body: B,
        buffer: BytesMut,
        decoder: StreamResponseDecoder<T>
    }
}

#[cfg(feature = "twurst-stream")]
impl<B: Body<Error: Error + Send + Sync + 'static>, T: ReflectMessage + Default> Stream
    for TwirpResponseStream<B, T>
{
    type Item = Result<T, TwirpError>;

    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        loop {
            let project = self.as_mut().project();
            if let Some(message) = project.decoder.read_next_message(project.buffer) {
                return Poll::Ready(Some(message));
            }
            let Some(frame) = ready!(project.body.poll_frame(cx)) else {
                return Poll::Ready(None);
            };
            let frame = match frame {
                Ok(frame) => frame,
                Err(e) => {
                    return Poll::Ready(Some(Err(TwirpError::wrap(
                        TwirpErrorCode::Internal,
            ...
            if let Ok(data) = frame.into_data() {
                self.as_mut().project().buffer.put(data) // TODO: remove already read data
...
#[cfg(feature = "twurst-stream")]
#[derive(Default)]
struct StreamResponseDecoder<T: ReflectMessage> {
    next_message_type_and_len: Option<(u8, u32)>,
    _output: PhantomData<T>,
}

#[cfg(feature = "twurst-stream")]
impl<T: ReflectMessage + Default> StreamResponseDecoder<T> {
    fn read_next_message(&mut self, buffer: &mut BytesMut) -> Option<Result<T, TwirpError>> {
        let (next_message_type, next_message_len) =
            if let Some(next_message_type_and_len) = self.next_message_type_and_len.take() {
                next_message_type_and_len
            } else {
                if buffer.remaining() < 5 {
                    return None;
                }
                (buffer.get_u8(), buffer.get_u32())
            };
        if buffer.remaining() < next_message_len as usize {
            self.next_message_type_and_len = Some((next_message_type, next_message_len)); // We save the value
            return None;
        }
        Some(match next_message_type {
            0 => T::decode(buffer).map_err(|e| {
                TwirpError::wrap(
                    TwirpErrorCode::Malformed,
                    format!("Bad response binary protobuf encoding: {e}"),
                    e,
            ...
            48 => {
                let next_message_len = next_message_len.try_into().unwrap();
                let error = serde_json::from_slice(&buffer[..next_message_len]);
                buffer.advance(next_message_len);
                Err(error.unwrap_or_else(|e| {
                    TwirpError::wrap(
                        TwirpErrorCode::Malformed,
                        format!("Invalid error serialization: {e}"),
                        e,
                    )
                }))
            }
            _ => Err(TwirpError::malformed(format!(
                "Invalid message type {next_message_type}"
            ))),
        })
    }
}
...
mod tests {
    ...
    async fn wrong_content_type_response() -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(
            response_error,
            TwirpError::malformed("Unsupported content type: foo/bar")
        );
        Ok(())
    }
